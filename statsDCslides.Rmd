...
```{r analytics,echo=FALSE,eval=FALSE,results='hide'}
library(caret)
library(gbm)
require(pROC)
testset<-sample(studat$stuid,500)
studat$year<-as.numeric(studat$year)
studat$proflvl<-as.numeric(studat$proflvl)
studat$case<-0
studat$case[studat$stuid %in% testset]<-1
training<-subset(studat,case==0)
testing<-subset(studat,case==1)
